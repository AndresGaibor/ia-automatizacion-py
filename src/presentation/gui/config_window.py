...
class ConfigWindow:
    ...
    def _test_connection(self):
        ...
        try:
            ...
            # Crear cliente API directamente con las credenciales de prueba
            base_url = 'https://acumbamail.com/api/1/'
            client = APIClient(base_url=base_url, auth_token=api_key)

            # Hacer una llamada simple para probar - usar el endpoint correcto de Acumbamail
            response = client._make_request('GET', 'getLists/')

            if response.status_code == 200:
                messagebox.showinfo("Éxito", "✅ Conexión exitosa con Acumbamail API")
            elif response.status_code == 401:
                messagebox.showerror("Error", "❌ API Key inválida. Verifique sus credenciales")
            else:
                messagebox.showerror("Error", f"❌ Error de conexión (HTTP {response.status_code}). Verifique sus credenciales")
